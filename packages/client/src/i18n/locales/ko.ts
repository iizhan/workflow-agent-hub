export default {
  ...
  chat: {
    ...
    sessions: '세션',
    webUiSessions: '세션',
    sessionScopeHint: '채팅에는 워크스페이스와 API 세션만 표시됩니다. CLI, Telegram, Discord, Cron 등 채널 세션은 기록에서 읽기 전용으로 볼 수 있습니다.',
    openHistory: '기록 열기',
    hermesHistory: '런타임 기록',
    historyScopeHint: '소스별로 그룹화된 런타임 기록 세션을 읽기 전용으로 봅니다.',
    noSessions: '세션 없음',
    newChat: '새 채팅',
  ...
  models: {
    ...
    copilotAddDetected: '이 컴퓨터에서 GitHub Copilot OAuth 토큰이 감지되었습니다. 추가를 클릭하여 현재 워크스페이스에서 Copilot을 활성화하세요.',
    ...
    copilotDeleteHintGhCli: 'Copilot이 현재 워크스페이스에서 숨겨집니다. gh CLI 로그인에는 영향이 없으며 `gh auth status`는 여전히 로그인 상태를 표시합니다.',
    copilotDeleteHintAppsJson: 'Copilot이 현재 워크스페이스에서 숨겨집니다. VS Code Copilot 확장 로그인에는 영향이 없습니다.',
    customBadge: '커스텀',
    previewBadge: '프리뷰',
    disabledBadge: '사용 불가',
  ...
  changelog: {
    ...
    new_0_5_15_9: '워크스페이스 Markdown 미디어 렌더링 및 동기화 재시도 수정',
