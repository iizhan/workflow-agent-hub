export default {
  ...
  chat: {
    ...
    emptyState: 'Inicie uma conversa com o assistente',
    ...
    sessionScopeHint: 'O chat mostra apenas sessões do workspace e da API. Sessões de CLI, Telegram, Discord, Cron e outros canais são somente leitura no Histórico.',
    openHistory: 'Abrir histórico',
    hermesHistory: 'Histórico de execução',
    historyScopeHint: 'Sessões do histórico de execução somente leitura, agrupadas por origem.',
...
jobTriggered: 'Job acionado',
  ...
  models: {
    ...
    copilotAddDetected: 'Foi detectado um token OAuth do GitHub Copilot nesta máquina. Clique em Adicionar para ativar o Copilot no workspace atual.',
    ...
    copilotDeleteHintGhCli: 'O Copilot ficará oculto neste workspace. Sua sessão no gh CLI não é afetada — `gh auth status` continuará indicando que está conectado.',
    copilotDeleteHintAppsJson: 'O Copilot ficará oculto neste workspace. A extensão Copilot do VS Code continuará conectada.',
  ...
  changelog: {
    ...
    new_0_5_15_9: 'Corrigido renderização de mídia do workspace e retry de sincronização',
    ...
    new_0_5_5_2: 'Adicionada página de histórico para sessões de execução',
